-- plan 1001  , work 1003 , transfer  1082
...
WITH mast_item as (
        ...
        , item_info.SHELF_LIFE_DAYS                                     SHELF_LIFE_DAYS  
...
--        , MSC_ANALYTIC_ORG_FLAT_V_DYD              mrp_store_org
    ...
    (
        SELECT mast_item.INVENTORY_ITEM_ID
           ...
           , ROUND(SUM(trx.TRANSACTION_QUANTITY)/(365/30),2 )             AVG_WEEK
        ...
        WHERE     
            ...
            AND (trx.TRANSACTION_DATE   BETWEEN   (:P_DATE - 365)   AND  :P_DATE)
        GROUP by mast_item.INVENTORY_ITEM_ID,po_header.VENDOR_SITE_ID,po_header.VENDOR_ID
    ),
    future_one-- 对应  I  J  库存和 在途     M  12  本周预测
    as
    (
        SELECT
            item_info.INVENTORY_ITEM_ID               ONE_INVENTORY_ITEM_ID
                ...
                -- )  as   ONE_PPO
                , SUM(
                CASE WHEN  orders.ORDER_TYPE = 1001 or  orders.ORDER_TYPE = 1003 or orders.ORDER_TYPE = 1082
                     THEN  orders.ORDER_QUANTITY
                     ELSE  0
                END  
                )  as   ONE_TD
...
--             AND (orders.FIRM_STATUS = 'Firm' or orders.ORDER_TYPE IN( 18,1,1001))
            AND orders.ORDER_TYPE IN( 18,1,1001,1003,1082)
--             AND orders.ORDER_TYPE IN( 1001,1003,1082)
...
--             AND orders.ORDER_TYPE IN( 18,1,1001,1003,1082)
            ...
            AND to_char(COALESCE(orders.FIRM_DATE,orders.SUGGESTED_DUE_DATE), 'YYYY-MM-DD')  IN
                                                        (    SELECT to_char(CALENDAR_DATE,'YYYY-MM-DD') set_time
            FROM MSC_ANALYTIC_CALENDARS_V
            where PERIOD_START_DATE  =  (select PERIOD_START_DATE
            from MSC_ANALYTIC_CALENDARS_V
            where CALENDAR_DATE = :P_DATE
                                                                )
                                                        )
...
--             AND orders.ORDER_TYPE IN( 18,1,1001,1003,1082)
            ...
            AND to_char(COALESCE(orders.FIRM_DATE,orders.SUGGESTED_DUE_DATE), 'YYYY-MM-DD')  IN
                                                        (    SELECT to_char(CALENDAR_DATE,'YYYY-MM-DD') set_time
            FROM MSC_ANALYTIC_CALENDARS_V
            where PERIOD_START_DATE  =  (select PERIOD_NEXT_DATE
            from MSC_ANALYTIC_CALENDARS_V
            where CALENDAR_DATE = :P_DATE
                                                                )
                                                        )
